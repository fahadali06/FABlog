...
<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Blogs in {{ $categoryname->title }} <button class="btn btn-primary btn-sm pull-right" onclick="OpenNew();">New Blogs</button></h1>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                {{ Form::open(['url' => url('admin/blogs/blog/store'), 'mehtod' => 'POST', 'id' => 'FormAdd', 'enctype' => 'multipart/form-data']) }}
                {{ Form::hidden('blog_category',$id) }}
                ...
                <div class="form-group">
                    ...
                    {{ Form::textarea('descriptionAdd', '', ['class' => 'form-control']) }}
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    {{ Form::label('descriptionEdit', 'Description') }}
                    {{ Form::textarea('descriptionEdit', '', ['class' => 'form-control']) }}
                    {{ Form::hidden('description') }}
                </div>
                <div class="form-group">
                    {{ Form::label('links', 'Link') }}<small>(optional)</small>
            ...
            <div class="modal-footer">
                <button onclick="FormEditSubmit();" id="FormEditSubmit" type="button" class="btn btn-primary">Save</button>
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function () {
        table = $('#dataTables-example').DataTable({
            ...
            "order": [[0, "desc"]],
            "ajax": {
                "url": "{{ url('admin/blogs/blog/ajax/list',[$id]) }}",
                "type": "GET",
            },
            "columns": [
                {"data": "id"},
    ...
    function FormAddSubmit(id) {
        if ($('#FormAdd input[name=title]').val() == "") {
            ...
            $('#FormAdd input[name=description]').val(CKEDITOR.instances['descriptionAdd'].getData());
            
            $.ajax({
                // Your server script to process the upload
                url: '{{ url("admin/blogs/blog/store") }}',
                type: 'POST',
                // Form data
    ...
    function edit(id) {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ url("admin/blogs/blog/edit") }}/' + id,
    ...
    function FormEditSubmit() {
        if ($('#FormEdit input[name=title]').val() == "") {
            ...
            $('#FormEdit input[name=description]').val(CKEDITOR.instances['descriptionEdit'].getData());

            $.ajax({
                // Your server script to process the upload
                url: '{{ url("admin/blogs/blog/update") }}/' + blog_id,
                type: 'POST',
                // Form data
    ...
    function delete_blog(id) {
        if (confirm('Do you want to do it ?')) {
            $.ajax({
                type: 'POST',
                url: '{{ url("admin/blogs/blog/delete") }}/' + id,
                //dataType: 'json',
